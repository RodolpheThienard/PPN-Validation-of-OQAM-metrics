...
static double
reduc (const double *restrict tab, const size_t size)
{
...
compute_routine (void *args)
{
    ...
    compute->elapsed = (double)(end.tv_sec - begin.tv_sec)
...
main (int argc, char *argv[])
{
    ...
    if (argc != 4)
        return fprintf (stderr,
                        "USAGE: %s nb_iteration nb_threads nb_chunks\n",
                        argv[0]),
               1;
            ...
            if (i == 0)
                {
                    ...
                    compute[i].nbIter
                        = (nbChunk - nbThrd + 1) * chunk + chunk % nbChunk;
                }
            else
                {
