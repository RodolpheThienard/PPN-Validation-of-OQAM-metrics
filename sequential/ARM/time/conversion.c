...
main (void)
{
    ...
    const size_t size = 1028 * 1028;
    int *a = aligned_alloc (64, size * sizeof (int));
    ...
    for (size_t i = 0; i < NB_REP; i++)
        {
            ...
            From_Int_To_Double (a, b, size);
            clock_gettime (CLOCK_MONOTONIC_RAW, &after);
            intToDoubleElapsed
                += (double)(after.tv_sec - before.tv_sec) * 100000000
